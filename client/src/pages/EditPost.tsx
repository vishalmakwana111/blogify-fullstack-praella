...
export function EditPost() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await postService.updatePost(id!, submitData);

      if (response.success) {
        navigate('/dashboard');
      } else {
        setMessage(response.message || 'Failed to update post');
      }
    } catch (error: any) {
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-600 mt-1 text-sm">Update your post content and settings</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {isLoading && formData.status === 'PUBLISHED' ? 'Updating...' : 'Update Post'}
              </button>
            </div>
          </form>
        </div>
